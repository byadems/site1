...
$(document).ready(function () {
    ...
    $('#ticket_talep').click(function () {
        ...
        $.post(window.admin_url + "/tickets", {
            ...
            if (data.status == 202) {
                $.post(window.admin_url + "/tickets/saglayici_ajax", {
    ...
    setInterval(function () {
        $.post(window.admin_url + '/tickets/ajax', {
            ticket: 1,
        }, function (data) {
            if (data.support_new) {
    ...
    $('#modalDiv').on('show.bs.modal', function (e) {
        ...
        $.post(window.admin_url + '/ajax_data', {
    ...
    $('#modalDivDuzenle').on('show.bs.modal', function (e) {
        $("#modalContent").html('<center><div class="modal-body"><div class="fa-3x"><i border="0" alt="loading" class="fas fa-spinner fa-spin"></i></div></div></center>');
        $.get(window.admin_url + '/services-detail/' + $(e.relatedTarget).data('id') + "?modal=1", function (data, status) {
...
function getProviderServices(provider, site_url) {
    if (provider == 0) {
        ...
        $.post(window.admin_url + '/ajax_data', {action: 'providers_list', provider: provider}).done(function (data) {
...
function getProviderServicesikili(provider, site_url) {
    if (provider == 0) {
        ...
        $.post(window.admin_url + '/ajax_data', {action: 'providers_list2', provider: provider}).done(function (data) {
