...
$o_id = $orderModel->db->escapeString($o_id);
...
if (route(2) != "all" && !route(3)):
    $orderModel->where('order_status', route(2));
...
if (!empty($_GET["search"])):
    ...
    $ss = $orderModel->db->escapeLikeString($ss);
    $search .= " && ( order_url LIKE '%" . $ss . "%' || order_id LIKE '%" . $ss . "%' ) ";
    //$orderModel->where('order_url', 'LIKE', '%' . $ss . '%');
...
if (!empty($_GET["subscription"])):
    ...
    $orderModel->where('subscriptions_id', 'LIKE', '%' . $_GET["subscription"] . '%');
endif;
if (!empty($_GET["dripfeed"])):
    $search .= " && ( dripfeed_id LIKE '%" . $_GET["dripfeed"] . "%'  ) ";
    $orderModel->where('dripfeed_id', 'LIKE', '%' . $_GET["dripfeed"] . '%');
endif;
...
try {
    ...
    $count = $conn->query("SELECT * FROM orders WHERE client_id='$c_id' && dripfeed='1' && subscriptions_type='1' ")->rowCount();

}
*/

$count = $orderModel->where('client_id', $c_id)->where('dripfeed', 1)->where('subscriptions_type', 1)->countAllResults();
...
try{
    ...
    $orders = $conn->prepare("SELECT * FROM orders INNER JOIN services WHERE services.service_id = orders.service_id && orders.dripfeed=:dripfeed && orders.subscriptions_type=:subs && orders.client_id=:c_id ORDER BY orders.order_id DESC LIMIT $where,$to ");
    $orders->execute(array("c_id" => $user["client_id"], "dripfeed" => 1, "subs" => 1));
    $orders = $orders->fetchAll(PDO::FETCH_ASSOC);
}
*/
...
->where('orders.client_id', $c_id)->where('orders.dripfeed', 1)->where('orders.subscriptions_type', 1)->orderBy('orders.order_id', 'DESC')->findAll($to, $where);
//INNER JOIN services WHERE services.service_id = orders.service_id
$ordersList = [];
