...
$title .= $languageArray["subscriptions.title"];
  ...
  if (route(2) != "all"):
      ...
      $orderModel->where("subscriptions_status",route(2));
  else:
      $search = "";
  endif;
...
$count = $orderModel->where('client_id', $c_id)->where('dripfeed', 1)->where('subscriptions_type', 2)->countAllResults();
...
$orders = $orderModel
    ...
    ->where('orders.client_id', $user["client_id"])->where('orders.dripfeed', 1)->where('orders.subscriptions_type', 2)->orderBy('orders.order_id', 'DESC')->findAll($to, $where);
