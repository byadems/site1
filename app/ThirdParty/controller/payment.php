...
if ($method_name == "shopier") :
    ...
    // REST API v1 formatı (shopier-notify.php webhook ile birlikte çalışır)
    // Bu callback eski OSB için: kullanıcı Shopier'dan geri yönlendirildiğinde çalışır
    if (!isset($post['platform_order_id']) && !isset($post['random_nr'])) :
        // REST API v1 kullanılıyorsa kullanıcı direkt ödeme sayfasından geri döner
        // Webhook (shopier-notify.php) bakiyeyi zaten yüklemiştir
        header("Location:" . base_url());
        exit;
    endif;
    ...
    $status = $post['status'];
    $payment_id_post = $post['payment_id'];
    $installment = $post['installment'];
    $random_nr         = $post['random_nr']          ?? '';
    $total_order_value = $post['total_order_value']  ?? '';
    $currency          = $post['currency']           ?? 0;

    // OSB imza dogrulama: random_nr + platform_order_id + total_order_value + currency
    $sig_data  = $random_nr . $order_id . $total_order_value . $currency;
    $signature = base64_decode($post['signature'] ?? '');
    $expected  = hash_hmac('SHA256', $sig_data, $extras['apiSecret'], true);
    ...
    if ($status == 1 || $status == '1') :
        // Atomik kontrol: payment_delivery=1 VE payment_status=1 olan kaydı bul
        $payment = $conn->prepare("SELECT p.*, c.balance AS c_balance FROM payments p INNER JOIN clients c ON c.client_id=p.client_id WHERE p.payment_privatecode=:orderid AND p.payment_delivery=1 AND p.payment_status=1 LIMIT 1");
        ...
        if ($payment) :
            ...
            // Row lock ile bakiyeyi kilitle
            $balQ = $conn->prepare("SELECT balance FROM clients WHERE client_id=:id FOR UPDATE");
            $balQ->execute(array("id" => $payment["client_id"]));
            $currentBalance = floatval($balQ->fetchColumn());

            // Atomik UPDATE: payment_delivery=1 koşuluyla güncelle (race condition koruması)
            $update = $conn->prepare("UPDATE payments SET client_balance=:balance, payment_status=3, payment_delivery=2, payment_extra=:extra WHERE payment_id=:id AND payment_delivery=1");
            $extra_data = json_encode($_POST);
            $updateResult = $update->execute(array("balance" => $currentBalance, "extra" => $extra_data, "id" => $payment["payment_id"]));

            if ($update->rowCount() === 0) :
                // Başka bir istek zaten işlemiş (concurrent)
                $conn->rollBack();
                header("Location:" . base_url());
                exit;
            endif;
            ...
            $balance = $balance->execute(array("id" => $payment["client_id"], "balance" => $currentBalance + $amount));
...
elseif ($method_name == 'coinpayments') :
    ...
    if ($status >= 100 || $status == 2) {
        ...
        if (countRow(['table' => 'payments', 'where' => ['client_id' => $user['client_id'], 'payment_status' => 1, 'payment_delivery' => 1, 'payment_extra' => $_POST['txn_id']]])) {
            if ($status >= 100 || $status == 2) {
...
$ch = curl_init('https://api.weepay.co/GetPayment/Detail?orderId=349645697305816407091109790166181415032488466646042457823128&paymentId=2932655');
...
$son = json_decode($response);

curl_close($ch);
if($son->status == "success"):
if($son->paymentStatus == "SUCCESS"):
...
if (isset($_POST['m_operation_id']) && isset($_POST['m_sign'])):
	...
	$sign_hash = strtoupper(hash('sha256', implode(':', $arHash)));
        ...
        if (!$err && $_POST['m_status'] == "success" && countRow(["table" => "payments", "where" => ["payment_privatecode" => $order_id, "payment_delivery" => 1,"payment_status" => 1]])) :
            ...
            // Referans BİTİŞ
            if ($payment_bonus) :
                $insert = $insert->execute(array("c_id" => $payment["client_id"], "action" => $order_id."|".$method["method_name"] . " API aracılığıyla %" . $payment_bonus["bonus_amount"] . " bonus dahil " . $amount . " TL tutarında bakiye yüklendi", "ip" => GetIP(), "date" => date("Y-m-d H:i:s")));
            else :
                $insert = $insert->execute(array("c_id" => $payment["client_id"], "action" => $method["method_name"] . " API aracılığıyla " . $amount . " TL tutarında bakiye yüklendi", "ip" => GetIP(), "date" => date("Y-m-d H:i:s")));
            endif;
...
elseif ($method_name == "stripe"):
    ...
    if(isset($_GET['session_id'])):
        ...
        \Stripe\Stripe::setApiKey($extras["secret"]); 
                try { 
...
elseif ($method_name == "paypal") :
    if (array_key_exists('paymentId', $_GET) && array_key_exists('PayerID', $_GET)):
        ...
        if ($response->isSuccessful()):
            ...
            if (!countRow(["table" => "payments", "where" => ["payment_privatecode" => $payment_id,"payment_status"=>1]])) :
