...
$categories = [];
$favoriteRows = $conn->prepare("SELECT * FROM client_favorite WHERE user_id = :userid ORDER BY id ASC");
$favoriteRows->execute(array('userid' => $user['client_id']));
$favoriteRowsCount = $favoriteRows->rowCount();
if (!$favoriteRowsCount) {

} else {
    $fav = array();
    $fav["category_name"] = "Favorilerim";
    $fav["category_id"] = "-1";
    array_push($categories, $fav);
}
...
foreach ($categoriesRows as $categoryRow) {
    ...
    if ($categoryRow["category_secret"] == 2 || $search->rowCount()):
        ...
        $c["icon"] = $categoryRow["icon"];
        $c["color"] = $categoryRow["color"];
        $c["services"] = $services;
...
if ($_POST):
    ...
    $quantity = isset($_POST["quantity"]) ? htmlspecialchars($_POST["quantity"]) : 1; // Sipariş miktarı
    ...
    $username = isset($_POST["username"]) ? htmlspecialchars($_POST["username"]) : ""; // abonelik, hangi kullanıcıya olacak
    $posts = isset($_POST["posts"]) ? htmlspecialchars($_POST["posts"]) : ""; // abonelik, kaç gönderiye gitsin
    $delay = isset($_POST["delay"]) ? htmlspecialchars($_POST["delay"]) : ""; // Abonelik, gecikme süresi
    $otoMin = isset($_POST["min"]) ? htmlspecialchars($_POST["min"]) : ""; // abonelik, minimum miktar
    $otoMax = isset($_POST["max"]) ? htmlspecialchars($_POST["max"]) : "";// abonelik, maksimum tutar
    $comments = isset($_POST["comments"]) ? htmlspecialchars($_POST["comments"]) : ""; //custom comments
    $order_username = isset($_POST["order_username"]) ? htmlspecialchars($_POST["order_username"]) : ""; //custom comments
    $answer_number = isset($_POST["answer_number"]) ? htmlspecialchars($_POST["answer_number"]) : ""; //custom comments
    $order_ext = array('answer' => $answer_number, 'username' => $order_username);
    $runs = isset($_POST["runs"]) ? htmlspecialchars($_POST["runs"]) : ""; // dripfeed kaç kez gitsin
    if (!$runs): $runs = 1; endif;

    //  if( $runs < 1 ): $runs = 1; endif;

    $interval = isset($_POST["interval"]) ? htmlspecialchars($_POST["interval"]) : ""; // dripfeed gecikme süresi
    $dripfeedon = isset($_POST["name"]) ? htmlspecialchars($_POST["name"]) : ""; // dripfeed aktif
    $expiry = isset($_POST["expiry"]) ? htmlspecialchars($_POST["expiry"]) : "";
    ...
    elseif ($service_detail["service_package"] == 4):
        $price = service_price($service_detail["service_id"]);
        if($service_detail["service_min"] == count(explode("\n", $comments))){
            $quantity = count(explode("\n", $comments));
        } else{
            $quantity = $service_detail["service_min"];
        }
        $extras = json_encode(["comments" => $comments]);
    elseif ($service_detail["service_package"] == 5):
        $extras = json_encode(["username" => $order_username]);
    elseif ($service_detail["service_package"] == 6):
        $extras = json_encode(["answer_number" => $answer_number]);
    elseif ($service_detail["service_package"] == 11 || $service_detail["service_package"] == 12 || $service_detail["service_package"] == 13):
        ...
        $get_id = file_get_contents("https://glycon.net/userid/userid.php?username=" . $ordername . "&glycon=xx");
    ...
    if ($service_detail["service_package"] == 14 || $service_detail["service_package"] == 15):
        $price = service_price($service_detail["service_id"]);
    elseif ($service_detail["service_package"] != 2 && $service_detail["service_package"] != 4 && $service_detail["service_package"] != 11 && $service_detail["service_package"] != 12 && $service_detail["service_package"] != 13):
    ...
    elseif ($service_detail["service_package"] == 5 && ($link == "" || $order_username == "")):
        $error = 1;
        $errorText = $languageArray["error.neworder.empty"];
    elseif ($service_detail["service_package"] == 6 && ($link == "" || $answer_number == "")):
        $error = 1;
        $errorText = $languageArray["error.neworder.empty"];
    ...
    elseif (($service_detail["service_package"] == 11 || $service_detail["service_package"] == 12 || $service_detail["service_package"] == 13) && $otoMax > $service_detail["service_max"]):
        $error = 1;
        $errorText = str_replace("{max}", $service_detail["service_max"], $languageArray["error.neworder.max"]);
    elseif (isset($service_detail["instagram_second"]) && $service_detail["instagram_second"] == 1 && (isset($countRow) && $countRow) && ($service_detail["service_package"] != 11 && $service_detail["service_package"] != 12 && $service_detail["service_package"] != 13 && $service_detail["service_package"] != 14 && $service_detail["service_package"] != 15)):
        $error = 1;
    ...
    elseif (($user["u_balance"] - $price < "-" . $user["debit_limit"]) && $user["balance_type"] == 1):
        ...
        if ($service_detail['rep_link'] == 2 && ( $order_ss->where(["client_id" => $user["client_id"], 'order_url' => $link, 'order_status!=' => 'completed','dripfeed' => 1, 'service_id' => $service_detail['service_id']])
        ->where([ 'order_status!=' => 'canceled'])
        ->where(['order_status!=' => 'partial'])
        ->countAllResults()|| $order_ss->where(["client_id" => $user["client_id"], 'order_url' => $link, 'dripfeed' => 2,'dripfeed_status!=' => 'canceled', 'service_id' => $service_detail['service_id']])
        ->where([ 'dripfeed_status!=' => 'completed'])
        ->countAllResults())) {

            $error = 1;
            $errorText = 'Aynı Linke Sipariş Bitmeden 1 kere verebilirsiniz';

        } else {/* Sipariş ver - başla */
            if ($service_detail["service_api"] == 0):
                ...
                if ($insert && $update && $insert2):
                    ...
                    $order_data = ['success' => 1, 'id' => $last_id, "service" => $service_detail["service_name"], "link" => $link, "quantity" => $quantity, "price" => bakiye_format($price), "balance" => $user["balance"]];
            ...
            /* manuel sipariş - bitir */
                ...
                if ($service_detail["service_package"] == 1 || $service_detail["service_package"] == 2):
                    ...
                    $bol = birlesme_bolme($quantity);
                    if (!$service_detail['birlestirme']) {
                    ...
                    } elseif ($service_detail["birlestirme"]) {
                        $order = $smmapi->action(array('key' => $api_detail["api_key"], 'action' => 'add', 'service' => $service_detail["api_service"], 'link' => $link, 'quantity' => $bol[0]), $api_detail["api_url"]);

                        if (!$service_detail['sirali_islem']) {
                            $api_v = new \App\Models\service_api();
                            $api_v_c = $api_v->where('id', $service_detail['service_api2'])->get()->getResultArray()[0];
                            $order2 = $smmapi->action(array('key' => $api_v_c['api_key'], 'action' => 'add', 'service' => $service_detail["api_service2"], 'link' => $link, 'quantity' => $bol[1]), $api_v_c["api_url"]);
                ...
                elseif ($service_detail["service_package"] == 5):
                    ...
                    $order = $smmapi->action(array('key' => $api_detail["api_key"], 'action' => 'add', 'service' => $service_detail["api_service"], 'link' => $link, 'quantity' => $quantity, "username" => $order_username), $api_detail["api_url"]);
                    if (!isset($order->order)):
                        $error = json_encode($order);
                        $order_id = "";
                ...
                elseif ($service_detail["service_package"] == 6):
                    ...
                    $order = $smmapi->action(array('key' => $api_detail["api_key"], 'action' => 'add', 'service' => $service_detail["api_service"], 'link' => $link, 'quantity' => $quantity, "answer_number" => $answer_number), $api_detail["api_url"]);
                    if (!isset($order->order)):
                        $error = json_encode($order);
                        $order_id = "";
                ...
                $posts = !$posts ? 0 : $posts;
                $delay = !$delay ? 0 : $delay;
                $otoMin = !$otoMin ? 0 : $otoMin;
                $otoMax = !$otoMax ? 0 : $otoMax;
              ...
              subscriptions_max=:max, subscriptions_expiry=:expiry, dripfeed_id=:dripfeed_id, api_charge=:api_charge, api_currencycharge=:api_currencycharge, order_profit=:profit,api_orderid2=:orders2,order_error2=:errors2,order_detail2=:detail2
              ");
                $insert = $insert->execute(array("count" => "0", "c_id" => $user["client_id"], "detail" => json_encode($order), "error" => $error, "s_id" => $service_detail["service_id"], "quantity" => $quantity, "price" => $price / $runs, "url" => $link,
                    "create" => date("Y.m.d H:i:s"), "extra" => $extras, "order_id" => $order_id, "last_check" => date("Y.m.d H:i:s"), "api" => $api_detail["id"], 'orders2' => isset($order_id2) ? $order_id2 : 0, 'errors2' => isset($error2) ? $error2 : 0, 'detail2' => isset($order2) ? json_encode($order2) : "",
                ...
                if ($insert && $update && ($order_id || $error) && $insert2):
                    ...
                    $conn->commit();
                    $user = $conn->prepare("SELECT * FROM clients WHERE client_id=:id");
                    // burası düzelecek $_SESSION["neira_userid"]
                    $user->execute(array("id" => $_SESSION["neira_userid"]));
                    $user = $user->fetch(PDO::FETCH_ASSOC);
                    $user['auth'] = 1;
                    $order_data = ['success' => 1, 'id' => $last_id, "service" => $service_detail["service_name"], "link" => $link, "quantity" => $quantity, "price" => bakiye_format($price), "balance" => $user["balance"]];
