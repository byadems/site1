				...
				<div class="card-header p-0">
					<div class="row align-items-center">
						<div class="col-md-12 navbar-expand-lg">
							...
							<a href="<?= base_url($settings['adminkey']."/tickets?search=unread")?>" class="btn-glycon btn-glycon-warning btn-card-header w-100 w-sm-auto text-center"><i class="fas fa-plus"></i> Okunmayanlar</a>
							<a href="<?= base_url($settings['adminkey']."/tickets?status=pending")?>" class="btn-glycon btn-glycon-danger btn-card-header w-100 w-sm-auto text-center"><i class="fas fa-plus"></i> Cevap Bekleyenler</a>
							<a href="<?= base_url($settings['adminkey']."/tickets")?>" class="btn-glycon btn-glycon-info btn-card-header w-100 w-sm-auto text-center"><i class="fas fa-plus"></i> Tümü</a>
...
<script src="https://cdn.datatables.net/1.11.3/js/jquery.dataTables.min.js"></script>
    <?php 
    $url = base_url($settings['adminkey'].'/getAllTickets');
    if(isset($_GET['status'])){
        
    $url = base_url($settings['adminkey'].'/getAllTickets?durum='.$_GET['status']);
    }elseif(isset($_GET['search'])){
    $url = base_url($settings['adminkey'].'/getAllTickets?durum='.$_GET['search']);
