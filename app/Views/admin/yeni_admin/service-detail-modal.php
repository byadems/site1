...
<div class="container">
                ...
                <strong class="fs-2 text-white" id="servis_ismi">
                    ...
                    foreach ($languages as $language) {
                        if ($language["default_language"]) {
                            echo $multiName[$language["language_code"]]!=""?$multiName[$language["language_code"]]:$serviceInfo["service_name"];
                        }
                    }
                    ?>
                    ...
                    <div class="tab-pane fade show active" id="ex1-tabs-1" role="tabpanel" aria-labelledby="ex1-tab-1">
                        <div class="col-md-12">
                            <form class="needs-validation form" action="<?php if (in_array($serviceInfo["service_package"], $abonelik)) {
                                echo base_url($settings['adminkey']."/services/edit-subscription/" . $serviceInfo["service_id"]);
                            } else {
                                echo base_url($settings['adminkey']."/services/edit-service/" . $serviceInfo["service_id"]."?modalsave=1");
                                ...
                                <div class="row g-3">
                                    ...
                                    <?php foreach ($languages as $language) {
                                        if ($language["default_language"]) { ?>
                                            <div class="col-sm-4">
                                                ...
                                                <input type="text" class="form-control" id="sevis_ismi_degis" onkeyup="servisismi()" name="name[<?= $language["language_code"] ?>]" placeholder="" value="<?= $multiName[$language["language_code"]]!=""?$multiName[$language["language_code"]]:$serviceInfo["service_name"] ?>" required="">
                                            </div>
                                        <?php } else { ?>
                                            <div class="col-sm-4">
                                                <label for="firstName" class="form-label">Servis İsmi
                                    ...
                                    <div class="col-sm-12">
                                        ...
                                        <textarea class="form-control" id="editor<?= $serviceInfo["service_id"]?>-<?= $rand?>" name="aciklama"><?= $serviceInfo['service_description'] ?></textarea>
                                    </div>
                                    <!-- **************ACIKLAMA SONU**************** -->
                                    <hr class="mb-0">
                                    <!-- **************MOD BASLANGIC**************** -->
    ...
    <script>
        ...
        function getProviderServices(provider, base_url) {
            if (provider == 0) {
                ...
                $.post(base_url + "/<?= $settings['adminkey']; ?>/ajax_data", {
    ...
    <script>
        ...
        function Fiyatdisable(thiss) {
            ...
            if(val == 1){
                $('select[name="dripfeed"]').prop("disabled", true);
                $('select[name="dripfeed"]').val("1");
            }else{
                $('select[name="dripfeed"]').prop("disabled", false);
            }
        }
...
var initSample<?= $serviceInfo["service_id"]?> = ( function() {
	var wysiwygareaAvailable = isWysiwygareaAvailable(),
		isBBCodeBuiltIn = !!CKEDITOR.plugins.get( 'bbcode' );

	return function() {
		var editorElement = CKEDITOR.document.getById( 'editor<?= $serviceInfo["service_id"]?>-<?= $rand?>' );

		// :(((

		// Depending on the wysiwygarea plugin availability initialize classic or inline editor.
		if ( wysiwygareaAvailable ) {
			CKEDITOR.replace( 'editor<?= $serviceInfo["service_id"]?>-<?= $rand?>' );
		} else {
			editorElement.setAttribute( 'contenteditable', 'true' );
			CKEDITOR.inline( 'editor<?= $serviceInfo["service_id"]?>-<?= $rand?>' );
...
initSample<?= $serviceInfo["service_id"]?>();
</script>
<script>
     tinymce.init({
            selector: 'textarea#tinymce<?= $serviceInfo["service_id"]?>-<?= $rand?>',
