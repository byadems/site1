    ...
    <div class="card apperance-hr">
        <div class="card-header p-0">
            <div class="row align-items-center">
                <div class="col-md-12">
                    <div class="card-header-title"><?= lang('Admin.langs'); ?></div>
                    <a href="<?= base_url($settings['adminkey'].'/appearance/language/new')?>" class="btn-glycon btn-glycon-success btn-card-header"><i class="fas fa-plus"></i> Yeni
                        <?= lang('Admin.addlang'); ?></a>
                </div>
            </div>
        </div>
        <div class="card-body">
                ...
                <div class="col-md-4">
                    <div class="glycon-module-card">
                        <div class="glycon-module-image">
                            ...
                            <a href="<?php echo base_url($settings['adminkey'].'/appearance/language/'.$language["language_code"]) ?>" class="btn-glycon btn-glycon-primary btn-glycon-module"><i class="fa fa-cog"
                                                                                                   aria-hidden="true"></i>
                                <?= lang('Admin.edit'); ?></a>
                            <a href="<?= ( $language["language_type"] == 1 )?base_url($settings['adminkey'].'/appearance/language/?lang-id='.$language["language_code"].'&lang-type=2'):base_url($settings['adminkey'].'/appearance/language/?lang-id='.$language["language_code"].'&lang-type=1')?>" class="btn-glycon btn-glycon-<?= $language["language_type"] == 2?'danger':'success' ?> btn-glycon-module"><i class="fas fa-<?= $language["language_type"] == 2?'times':'check' ?>"
                                                                                                  aria-hidden="true"></i>
                                <?php if( $language["language_type"] == 2 ): echo  lang('Admin.passive');else: echo lang('Admin.activated');endif;  ?></a> 
                                <?php if(!$language['default_language'] && $language["language_type"] == 2): ?>
                                <a href="<?= base_url($settings['adminkey'].'/appearance/language/?lang-id='.$language["language_code"].'&lang-default=1&lang-short='.$language["language_code"])?>" class="btn-glycon btn-glycon-success btn-glycon-module"><i class="fas fa-<?= $language["language_type"] == 2?'times':'check' ?>"
                                                                                                  aria-hidden="true"></i>
                                <?= lang('Admin.actiondefault'); ?></a>
                                <?php endif; ?>
                        </div>
                    </div>
                </div>
...
<div class="col-md-10">
			<div class="card apperance-hr">
				<div class="card-header p-0">
					<div class="row align-items-center">
						<div class="col-md-12">
							<div class="card-header-title"><?= lang('Admin.turkish'); ?></div>
							<a href="<?= base_url($settings['adminkey']."/appearance/language") ?>" class="btn-glycon btn-glycon-success btn-card-header"><i class="fas fa-arrow-left"></i> Geri</a>
						             <div class="col-md-6 text-right">
                                <div class="pull-right">
  <input class="form-control" placeholder="<?= lang('Admin.searchword'); ?>"  id="myInput" onkeyup="myFunction()" type="text" value="">                                </div>
                            </div>
                        </div>
					</div>
				</div>
				<div class="card-body">
					<div class="row">
								<form action="<?php echo base_url($settings['adminkey'].'/appearance/language/new') ?>" method="post" enctype="multipart/form-data">
						<div class="col-12">
							<div class="language-detail" style="max-height: 600px;overflow-y: scroll;padding:5px 10px;">
									<div class="form-group">
										<label class="control-label"><?= lang('Admin.langname'); ?></label>
										<input type="text" class="form-control" name="language" value="<?= lang('Admin.turkish'); ?>">
									</div>
                                     <div class="form-group">
              <label class="control-label"><?= lang('Admin.langkey'); ?></label>
...
<div id="myUL">
							...
							<button type="submit" class="glycon-badge badge-success pointer text-decoration-none my-3 w-100"><?= lang('Admin.add'); ?></button>
								</form>
						</div>
					</div>
				</div>
...
<div class="col-md-10">
			<div class="card apperance-hr">
				<div class="card-header p-0">
					<div class="row align-items-center">
						<div class="col-md-12">
							<div class="card-header-title"><?= lang('Admin.turkish'); ?></div>
							<a href="<?= base_url($settings['adminkey']."/appearance/language") ?>" class="btn-glycon btn-glycon-success btn-card-header"><i class="fas fa-arrow-left"></i> <?= lang('Admin.back'); ?></a>
						             <div class="col-md-6 text-right">
  ...
  <input class="form-control" placeholder="<?= lang('Admin.searchword'); ?>"  id="myInput" onkeyup="myFunction()" type="text" value="">                                </div>
				...
				<div class="card-body">
					<div class="row">
								<form class="form" data-xhr="true" action="<?php echo base_url($settings['adminkey'].'/appearance/language/'.route(4)) ?>" method="post" enctype="multipart/form-data">
						<div class="col-12">
							<div class="language-detail" style="max-height: 600px;overflow-y: scroll;padding:5px 10px;">
									<div class="form-group">
										<label class="control-label"><?= lang('Admin.langname'); ?></label>
							...
							<button type="submit" class="glycon-badge badge-success pointer text-decoration-none my-3 w-100"><?= lang('Admin.update'); ?></button>
								</form>
						</div>
					</div>
				</div>
