					...
					<div class="card">
						...
						<div class="card-body">
							...
							<div class="d-grid gap-2">
								<a href="<?php echo base_url($settings['adminkey'].'/appearance/themes/'.$theme["theme_dirname"]) ?>" class="btn-glycon btn-glycon-primary text-center"><i class="fas fa-check"></i> <?= lang('Admin.edit'); ?></a>
								<a href="<?php echo base_url($settings['adminkey'].'/appearance/themes/active/'.$theme["theme_dirname"]) ?>" class="btn-glycon btn-glycon-<?= $settings["site_theme"] != $theme["theme_dirname"]?"success":"danger"?> text-center"><i class="fas fa-<?= $settings["site_theme"] != $theme["theme_dirname"]?"check":"times"?>"></i> <?= $settings["site_theme"] != $theme["theme_dirname"]?  lang('Admin.themesactiveaction'):lang('Admin.themesactive') ?></a>
							</div>
						</div>
					</div>
				</div>
...
<?php elseif( route(4) && route(4) != "active"):
    $aranacak_string = "/";
    $arama_sonucu=@strstr($_GET[file],$aranacak_string);
if($arama_sonucu!==FALSE){
    header("Location:" . base_url($settings['adminkey'].'/appearance/themes/'.$theme["theme_dirname"]));
    }
...
?>
 <div class="col-md-10">
    <div class="panel">
      <div class="panel-heading edit-theme-title"><strong><?php echo $theme["theme_name"] ?></strong> <?= lang('Admin.themeseditting'); ?></div>
              ...
              <div class="twig-editor-block">
                ...
                foreach ($layouts as $style => $layout):
                  ...
                  foreach ($layouts[$style] as $layout) :
                    ...
                    echo '
                      <li '. $active .'><a href="'.base_url($settings['adminkey'].'/appearance/themes/'.$theme["theme_dirname"]).'?file='.$layout.'">'.$layout.'</a></li>';
                  endforeach;
                  echo '</ul>';
                endforeach;
              ?>
          ...
          <div class="col-md-9 padding-md-left-null edit-theme__block-editor">
            ...
            <?php else: ?>
               ...
               <div class="panel-body">
                ...
                <form action="<?php echo base_url($settings['adminkey']."/appearance/themes/".$theme["theme_dirname"]."?file=".$lyt) ?>" method="post" class="twig-editor__base_url">

                  <textarea id="code" name="code" class="codemirror-textarea"><?=$text;?></textarea>
                  <div class="edit-theme-body-buttons text-right">
                    <button class="btn btn-primary click"><?= lang('Admin.update'); ?></button>
                  </div>
                </form>
...
$.post(site_url+'/<?=$settings['adminkey']?>/ajax_data',{action:'service-sortable',services:array});
}
});
...
$.post(site_url+'/<?=$settings['adminkey']?>/ajax_data',{action:'paymentmethod-sortable',methods:array});
}
});
...
$.post(site_url+'/<?=$settings['adminkey']?>/ajax_data',{action:'category-sortable',categories:array});
}
});
