...
<div class="container-fluid px-sm-5">
    <div class="row">
        <div class="col-md-12 collapse ms-auto mb-3" id="filter">
            <a href="<?= base_url(); ?>/<?= $settings['adminkey']; ?>/orders"
        ...
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body mobile-overflow p-0">
                         <form id="changebulkForm" action="<?php echo base_url($settings['adminkey']."/orders/multi-action") ?>"
                              method="post">
                        ...
                        <form id="changebulkForm" action="<?php echo base_url($settings['adminkey']."/orders/multi-action") ?>"
                            ...
                            <?php foreach ($orders as $order): ?>
                                <tr>
                                    ...
                                    <td><a href="<?= base_url() ?>/<?= $settings['adminkey']; ?>/client-detail/<?= $order["client_id"]; ?>"
                                           target="_blank"
                                        ...
                                        <div class="dropdown text-center">
                                            ...
                                            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                                                <li><a class="dropdown-item"
                                                       href="<?= base_url($settings['adminkey']."/order-detail/" . $order['order_id']) ?>">Servis
                                                        Detayı</a></li>
                                                <hr>
                                                <?php if ($order["order_error"] != "-" && $order["service_api"] != 0): ?>
                                                    ...
                                                    <li>
                                                        <a class="dropdown-item"
                                                           href="<?= base_url($settings['adminkey']."/orders/order_resend/" . $order["order_id"]) ?>">Yeniden
                                                            Gönder</a></li>
                                                <?php endif; ?>
                                                <hr>
                                                <?php if ($order["order_error"] == "-" && $order["service_api"] != 0): ?>
                                                ...
                                                <li>
                                                    <a class="dropdown-item"
                                                       ...
                                                       data-href="<?= base_url($settings['adminkey']."/orders/order_cancel/" . $order["order_id"]) ?>">İptal
                                                        ve İade Et</a>
                                                </li>
                                                <li>
                                                    <a class="dropdown-item" href="#"
                                                       data-bs-toggle="modal" data-bs-target="#confirmChange"
                                                       data-href="<?= base_url($settings['adminkey']."/orders/order_complete/" . $order["order_id"]) ?>">Tamamlandı</a>
                                                </li>
                                                <li>
                                                    <a class="dropdown-item" href="#"
                                                       data-bs-toggle="modal" data-bs-target="#confirmChange"
                                                       data-href="<?= base_url($settings['adminkey']."/orders/order_inprogress/" . $order["order_id"]) ?>">Yükleniyor</a>
                                                </li>
                                            </ul>
                                        </div>
                                    </td>
...
<?php
$url = $subs!=""?base_url($settings['adminkey'].'/getallOrdersSubs?subs='.$subs):base_url($settings['adminkey'].'/getallOrders');
...
<script>
    ...
    $('#manuel_siparis').click(function(){
        ...
        $('#orders tbody').empty();
        tablo_olustur("<?= base_url($settings['adminkey'].'/getallOrders?mode=manuel')?>");
    });
    $('#pending_siparis').click(function(){
        $('#orders').DataTable().destroy();
        $('#orders tbody').empty();
        tablo_olustur("<?= base_url($settings['adminkey'].'/getallOrders?mode=pending')?>");
    });
    $('#inprogress_siparis').click(function(){
        $('#orders').DataTable().destroy();
        $('#orders tbody').empty();
        tablo_olustur("<?= base_url($settings['adminkey'].'/getallOrders?mode=inprogress')?>");
    });
    $('#processing_siparis').click(function(){
        $('#orders').DataTable().destroy();
        $('#orders tbody').empty();
        tablo_olustur("<?= base_url($settings['adminkey'].'/getallOrders?mode=processing')?>");
    });
    $('#completed_siparis').click(function(){
        $('#orders').DataTable().destroy();
        $('#orders tbody').empty();
        tablo_olustur("<?= base_url($settings['adminkey'].'/getallOrders?mode=completed')?>");
    });
    $('#partial_siparis').click(function(){
        $('#orders').DataTable().destroy();
        $('#orders tbody').empty();
        tablo_olustur("<?= base_url($settings['adminkey'].'/getallOrders?mode=partial')?>");
    });
    $('#canceled_siparis').click(function(){
        $('#orders').DataTable().destroy();
        $('#orders tbody').empty();
        tablo_olustur("<?= base_url($settings['adminkey'].'/getallOrders?mode=canceled')?>");
    });
    $('#fail_siparis').click(function(){
        $('#orders').DataTable().destroy();
        $('#orders tbody').empty();
        tablo_olustur("<?= base_url($settings['adminkey'].'/getallOrders?mode=fail')?>");
    });
</script>
