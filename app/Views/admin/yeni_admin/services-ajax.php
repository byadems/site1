...
<div class="card-body p-0 mobile-overflow">
    ...
    <form action="<?php echo base_url($settings['adminkey']."/services/multi-action") ?>" method="post"
          id="changebulkForm">
                            ...
                            <div class="btn-group float-end">
                                ...
                                <ul class="dropdown-menu">
                                    ...
                                    <li><a class="dropdown-item"
                                           href="<?php echo base_url($settings['adminkey']."/services/" . $type . "/" . $service[0]["category_id"]) ?>"><?php if ($service[0]["category_type"] == 1): echo "Kategoriyi Aktifleştir"; else: echo "Kategoriyi Pasifleştir"; endif; ?></a>
                                    </li>
                                    <?php if (!countRow(['table' => 'services', 'where' => ['category_id' => $service[0]["category_id"]]])): ?>
                                        <li><a class="dropdown-item"
                                               href="<?php echo base_url($settings['adminkey']."/services/del_cate/" . $service[0]["category_id"]) ?>">Kategoriyi
                                                Sil</a></li>
                                    <?php endif; ?>
                                    <li><a class="dropdown-item"
                                    ...
                                    <?php if (!$service[0]['price_line']): ?>
                                        <li><a class="dropdown-item"
                                               href="<?php echo base_url($settings['adminkey']."/services/fiyat_sirala/" . $service[0]["category_id"]) ?>">Fiyata
                                                Göre Sıralat </a></li>
                                    <?php else: ?>
                                        <li><a class="dropdown-item"
                                               href="<?php echo base_url($settings['adminkey']."/services/fiyat_sirala_iptal/" . $service[0]["category_id"]) ?>">Fiyata
                                                Göre Sıralayı iptal et </a></li>
                                        <?php if (!$service[0]['fiyat_siralama']): ?>
                                            <li><a class="dropdown-item"
                                                   href="<?php echo base_url($settings['adminkey']."/services/fiyat_sirala_az/" . $service[0]["category_id"]) ?>">Fiyata
                                                    Azdan Çoğa Göre Sırala </a></li>
                                        <?php else: ?>
                                            <li><a class="dropdown-item"
                                                   href="<?php echo base_url($settings['adminkey']."/services/fiyat_sirala_cok/" . $service[0]["category_id"]) ?>">Fiyata
                                                    Çoktan Aza Göre Sırala </a></li>
                                        <?php endif; ?>
                                    <?php endif; ?>
                                    <!--
                                                    <li><a class="dropdown-item"
                                                           href="<?php echo base_url($settings['adminkey']."/services/" . $type . "/" . $service[0]["category_id"]) ?>"><?php if ($service[0]["category_type"] == 1): echo "Fiyat Belirle"; else: echo "Fiyat Belirle"; endif; ?></a>
                                                    </li>
													<li><a class="dropdown-item"
                                                           href="<?php echo base_url($settings['adminkey']."/services/" . $type . "/" . $service[0]["category_id"]) ?>"><?php if ($service[0]["category_type"] == 1): echo "Açıklama Belirle"; else: echo "Açıklama Belirle"; endif; ?></a>
                                                    </li> -->
                                </ul>
                            </div>
                        </th>
...
<script>
    ...
    window.url = "<?= base_url($settings['adminkey'].'/gizle-ajax')?>?category=" + window.bol[0];
    icerik_cek();
    ...
    $(".category-sortable").sortable({
        ...
        update: function (event, ui) {
            ...
            $.post(base_url + '/<?=$settings['adminkey']?>/ajax_data', {action: 'category-sortable', categories: array});
        }
    });
    ...
    */
    ...
    $(".methods-sortable").sortable({
        ...
        update: function (event, ui) {
            ...
            $.post(base_url + '<?=$settings['adminkey']?>/ajax_data', {action: 'paymentmethod-sortable', methods: array});
        }
    });
