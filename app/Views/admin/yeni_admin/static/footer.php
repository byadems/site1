    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body" style="padding: 0px;">
                <iframe src="<?php echo base_url($settings['adminkey']."/manager"); ?>"
...
<script>
    ...
    $(document).ready(function () {
        ...
        window.admin_sub = '<?= $settings['adminkey']?>';
        window.admin_url = window.base_url+"/"+window.admin_sub
    });
</script>
<!-- Bootstrap -->
...
<!-- CodeMirror -->
<!-- FontAwesome -->
<script type="text/javascript" src="https://kit.fontawesome.com/7b04a59200.js"></script>
<!-- FontAwesome -->
...
getProviderServices($("#provider").val(),base_url);
        ...
        $(".service-sortable").sortable({
            ...
            update: function (event, ui) {
                ...
                $.post(base_url + '/<?=$settings['adminkey']?>/ajax_data', {action: 'service-sortable', services: array});
            }
        });
...
<script>
    function guncelleme_denetle() {
        $.post("<?= base_url($settings['adminkey']."/version_check")?>", {}, function (data) {
            if (data.status == 200) {
                if (data.current_version != data.new_version) {
                    ...
                    swal({
                        ...
                        .then(name => {
                            if (!name) throw null;
                            fetch("<?= base_url($settings['adminkey']."/update_version")?>");
                            fetch("<?= base_url($settings['adminkey']."/create_db_update")?>");
                            return fetch("<?= base_url($settings['adminkey']."/version_update_settings")?>");
                        })
                        .then(results => {
                            return results.json();
                        })
