...
<script src="https://cdn.ckeditor.com/4.22.1/standard/ckeditor.js"></script>
    <script src="https://kit.fontawesome.com/7b04a59200.js" crossorigin="anonymous"></script>
...
<body class="bg-light">
            ...
            <div class="row">
                <div class="col-12 col-md-4">
                    <div class="row align-items-center">
                        <div class="col-md-8">
                            <a href="<?= base_url('') ?>/<?= $settings['adminkey']; ?>"><img style="filter: invert(1%) sepia(1%) saturate(1%) hue-rotate(1deg) brightness(1000%) contrast(100%);" class="img-fluid glycon heartbeat" src="<?= base_url('') ?>/image/glycon.png"></a>
                            <?php if ($user['access']['general_settings']) : ?><a href="javascript::void(0)" onclick="guncelleme_denetle();" class="mx-2 my-2 text-end topbar-a text-success"></i>
                                    Güncelleme Denetle</a><?php endif; ?>
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav">
                    <li class="nav-item border-right">
                        <a class="nav-link" aria-current="page" href="<?= base_url($settings['adminkey'] . "") ?>"><i class="fas fa-home"></i>
                            Ana Sayfa</a>
                    </li>
                    <?php if ($user['access']['orders']) : ?>
                        <li class="nav-item border-right">
                            <a class="nav-link" href="<?= base_url($settings['adminkey'] . '/orders') ?>"><i class="fas fa-boxes"></i>
                                Siparişler</a>
                        </li>
                    <?php endif; ?>
                    <?php if ($user['access']['subscriptions']) :
                        ...
                        <?php if ($a1 > 0 || $a2 > 0 || $a3 > 0 || $a4 > 0 || $a5 > 0) : ?>
                            <li class='nav-item border-right'><a class="nav-link" href='<?php echo base_url($settings['adminkey'] . '/subscriptions') ?>'><i class="fas fa-clock"></i> Abonelikler</a>
                            </li>
                    <?php endif;
                    endif; ?>
                    <?php if ($user['access']['dripfeed']) : ?>
                        <?php if (countRow(['table' => 'services', 'where' => ['service_dripfeed' => 2]]) > 0) :    ?>
                            <li class='nav-item border-right'><a class="nav-link" href='<?php echo base_url($settings['adminkey'] . '/dripfeeds') ?>'> Drip-feed</a></li>
                    <?php endif;
                    endif; ?>
                    <?php if ($user['access']['users']) : ?>
                        <li class="nav-item border-right">
                            <a class="nav-link" href="<?= base_url($settings['adminkey'] . '/clients') ?>"><i class="fas fa-users"></i>
                                Müşteriler</a>
                        </li>
                    <?php endif; ?>
                    <?php if ($user['access']['services']) : ?>
                        <li class="nav-item border-right">
                            <a class="nav-link" href="<?= base_url($settings['adminkey'] . '/services') ?>"><i class="fas fa-stream"></i>
                                Servisler</a>
                        </li>
                    <?php endif; ?>
                    <?php if ($user['access']['tasks']) : ?>
                        <?php if (countRow(['table' => 'tasks'])) : ?>
                            <li class="nav-item border-right">
                                <a class="nav-link" href="<?= base_url($settings['adminkey'] . '/tasks') ?>"><i class="fas fa-tasks"></i>
                    ...
                    <?php if ($user['access']['reports']) : ?>
                        <li class="nav-item border-right">
                            <a class="nav-link" href="<?= base_url($settings['adminkey'] . '/reports') ?>"><i class="fas fa-chart-line"></i>
                                <strong>G</strong>Analytics</a>
                        </li>
                    <?php endif; ?>
                    <?php if ($user['access']['payments']) : ?>
                        <li class="nav-item border-right">
                            <a class="nav-link" href="<?= base_url($settings['adminkey'] . '/payments') ?>"><i class="fas fa-coins"></i>
                    ...
                    <?php if ($user['access']['tickets']) : ?>
                        <li class="nav-item border-right">
                            <a class="nav-link" href="<?= base_url($settings['adminkey'] . '/tickets') ?>"><i class="fas fa-headset"></i>
                                Destek
                    ...
                    <?php endif; ?>
                    <?php if ($user['access']['child_panels']) : ?>
                        <?php if ($settings['panel_selling'] == 2 || countRow(['table' => 'child_panels', 'where' => ['panel_status' => 'active']])) : ?>
                            <li class="nav-item border-right"><a class="nav-link" href='<?php echo base_url($settings['adminkey'] . '/child-panels') ?>'>Child Panels
                        ...
                        <?php endif; ?>
                    <?php endif; ?>
                    <?php if ($user['access']['pages'] || $user['access']['news'] || $user['access']['blog'] || $user['access']['menu'] || $user['access']['themes'] || $user['access']['language'] || $user['access']['files']) : ?>
                        <li class="nav-item border-right">
                            <a class="nav-link" aria-current="page" href="<?= base_url($settings['adminkey'] . '/appearance') ?>"><i class="fab fa-atlassian"></i> Görünüm</a>
                        </li>
                    <?php endif; ?>

                    <?php if ($user['access']['general_settings'] || $user['access']['providers'] || $user['access']['integrations'] || $user['access']['payments_settings'] || $user['access']['modules'] || $user['access']['alert_settings'] || $user['access']['payments_bonus'] || $user['access']['bank_accounts']) : ?>
                        <li class="nav-item border-right">
                            <a class="nav-link" aria-current="page" href="<?= base_url($settings['adminkey'] . '/settings') ?>"><i class="fas fa-cogs"></i> Ayarlar</a>
                        </li>
                    <?php endif; ?>

                    <?php if ($user['access']['alert_settings']) : ?>
                        <li class="nav-item border-right">
                            <a class="nav-link" aria-current="page" href="<?= base_url($settings['adminkey'] . '/mail-sablon') ?>"><i class="fas fa-envelope-open"></i> Mail Şablonu</a>
                        </li>
                    <?php endif; ?>

                    <?php if ($user['access']['popup']) : ?>
                        <li class="nav-item border-right">
                            <a class="nav-link" aria-current="page" href="<?= base_url($settings['adminkey'] . '/popup') ?>"><i class="fas fa-fire"></i> Popup</a>
                        </li>
                    <?php endif; ?>

                    <?php if ($user['access']['logs']) : ?>
                        <li class="nav-item">
                            <a class="nav-link" href="<?= base_url($settings['adminkey'] . '/logs') ?>"><i class="fas fa-book-open"></i> Loglar</a>
                        </li>
                    <?php endif; ?>
    ...
    $kaynak = file_get_contents("https://i62.net/classes/alert2.php");
    if ($kaynak == true) {
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="alert alert-danger">
                                ...
                                <?= $data['0']['data'] ?> <?php if ($data['0']['ozelactive'] == 1) { ?>
                                <a href="<?= base_url($settings['adminkey'].'/'.$data['0']['ozelveri'])?>"><?= $data['0']['ozelveri_baslik'] ?></a>
                                <?php } ?> 
                                <?php if ($data['0']['ozelactive'] == 2) { ?>
                                <a href="//<?= $data['0']['ozelveri'] ?>"><?= $data['0']['ozelveri_baslik'] ?></a>
                                <?php } ?>
                            </div>
                        </div>
                    </div>
