        ...
        <div class="col-md-12">
            <div class="card">
               ...
               if($ticketMessage[0]["support_new"] == 2){
                ...
                <div class="card-body">
                    <form action="<?php echo base_url($settings['adminkey']."/tickets/read/" . $ticketMessage[0]["ticket_id"]) ?>"
                            ...
                            <div class="col-md-2 order-3 order-sm-1">
                                <div class="card mt-3 mt-sm-0 text-center">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <a href="<?= base_url($settings['adminkey']."/client-detail/".$ticketMessage[0]['client_id'])?>" target="_blank" class="btn-glycon btn-glycon-primary btn-block">Üye Profili</a>
                                    </div>
                                    <div class="card-footer text-muted">
                                        <?php
                                        $ticketSender = $ticketMessage[0]['username'];
                                        ?>
                                        <a href="<?php echo base_url($settings['adminkey']."/tickets/?search=$ticketSender&search_type=client"); ?>"
                            ...
                            <div class="col-md-2 order-2 order-sm-3">
                                <div class="card mt-3 mt-sm-0 text-center">
                                    ...
                                    <div class="card-body  text-start py-3">
                                        ...
                                        <select class="w-100" id="talep_degis">
...
<script>
    $('#talep_degis').on('change', function (e) {
        console.log('x')
        var data = $(this).val();
        var id = "<?= $ticketMessage[0]["ticket_id"] ?>";
        console.log(id)
        $.post(window.admin_url + '/tickets/read/' + id, {
            durum: data,
            id: id
        }, function (data) {
            $.toast({
...
<script>
    editor = CKEDITOR.replace( 'editor2' );; //fck is just my instance name you will need to replace that with yours
...
<script>
    $('#hazir_cevap').change(function () {
        var value = $(this).val();
        $.post(window.base_url + "/<?= $settings['adminkey']; ?>/tickets/getReady", {id: value}, function (data) {
    ...
    $('#confirmYes').click(function () {
        ...
        $.post(window.base_url + "/<?= $settings['adminkey']; ?>/tickets/ready", {content: contents, title: titles}, function (data) {
            if (data.m == "okey") {
                $.toast({
                    heading: "Hazır Cevap",
    ...
    $('#hazir_cevap_sil').click(function () {
        ...
        if (value != "-1") {
            $.post(window.base_url + "/<?= $settings['adminkey']; ?>/tickets/deleteReady", {id: value}, function (data) {
                $.toast({
                    heading: "Hazır Cevap",
