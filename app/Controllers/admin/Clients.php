...
class Clients extends Ana_Controller
{
    ...
    function index()
    {
        ...
        global $conn;
        global $_SESSION;
        $access = $this->getuser["access"]["users"];
        if (!$access) {
        ...
        $referrer = base_url($settings['adminkey'] . '/clients');
        if (!route(3)) :
            $page = 1;
        ...
        elseif ($action == "new" && $action) :
            ...
            if ($_POST) :
                ...
                if (!email_check($email)) {
                    ...
                    $insert = $conn->prepare("INSERT INTO clients SET first_name=:name, last_name=:lname, balance_type=:balance_type, debit_limit=:debit_limit,  username=:username, email=:email, password=:pass, telephone=:phone, register_date=:date, apikey=:key ");
                    $insert = $insert->execute(array("name" => $isim, "lname" => $soyisim, "debit_limit" => $debit_limit, "balance_type" => $debit, "username" => $username, "email" => $email, "pass" => md5(sha1(md5($pass))), "phone" => $tel, "date" => date("Y.m.d H:i:s"), 'key' => $apikey));
                    if ($insert) :
                        $conn->commit();
                        $referrer = base_url($settings['adminkey'] . "/clients");
                        $error = 1;
        ...
        elseif ($action == "edit" && $action) :
            $user = $this->getuser;
            $username = route(4);
            if (!countRow(["table" => "clients", "where" => ["username" => $username]])) : header("Location:" . base_url($settings['adminkey'] . "/clients"));
                exit();
            endif;
            ...
            if ($_POST) :
                ...
                if (!email_check($email)) {
                    ...
                    $apikey = CreateApiKey($_POST);
                    $conn->beginTransaction();
                    $insert = $conn->prepare("UPDATE clients SET first_name=:name, last_name=:lname, username=:username, balance_type=:balance_type, debit_limit=:debit_limit,  email=:email, telephone=:phone, register_date=:date WHERE username=:id ");
                    $insert = $insert->execute(array("id" => route(4), "name" => $isim, "lname" => $soyisim, "username" => $usernagme, "balance_type" => $debit, "debit_limit" => $debit_limit, "email" => $email, "phone" => $tel, "date" => date("Y.m.d H:i:s")));
        ...
        elseif ($action == "active" && $action) :
            ...
            $client_id = route(4);
            if (countRow(["table" => "clients", "where" => ["client_id" => $client_id, "client_type" => 2]])) : header("Location:" . base_url($settings['adminkey'] . "/clients"));
                exit();
            endif;
            ...
            return redirect()->to(base_url($settings['adminkey'] . "/clients"));
        elseif ($action == "deactive" && $action) :
            $client_id = route(4);
            if (countRow(["table" => "clients", "where" => ["client_id" => $client_id, "client_type" => 1]])) : header("Location:" . base_url($settings['adminkey'] . "/clients"));
                exit();
            endif;
            $update = $conn->prepare("UPDATE clients SET client_type=:type WHERE client_id=:id ");
        ...
        elseif ($action == "del_price" && $action) :
            ...
            if (!countRow(["table" => "clients_price", "where" => ["client_id" => $client_id]])) : $_SESSION["client"]["data"]["error"] = 1;
                ...
                header("Location:" . base_url($settings['adminkey'] . "/clients"));
                exit();
            endif;
    ...
    function detail()
    {
        ...
        $user = new \App\Models\clients();
        $getdduser1 = $user->where('client_id', $session->get('neira_userid'))->get()->getResultArray()[0];
        $getdduser1['access'] = json_decode($getdduser1['access'], true);

        $getdduser = $user->where('client_id', $this->request->uri->getSegment(3))->get()->getResultArray()[0];
        $getdduser['access'] = json_decode($getdduser['access'], true);

        if (!$getdduser1['access']['users']) {
            header("Location:" . base_url($this->settings['adminkey']));
        } else {
            if ($getdduser['access']['usersnoaccess']) {
                if (!$getdduser1['access']['usersnoaccess']) {
                    header("Location:" . base_url($this->settings['adminkey']));
                }
            }
        }
