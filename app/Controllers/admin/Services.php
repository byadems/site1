...
class Services extends Ana_Controller
{
    ...
    function index()
    {
        ...
        include APPPATH . 'ThirdParty/SMMApi/SMMApi.php';
                $access = $this->getuser["access"]["services"];
        if (!$access) {
            header("Location:" . base_url($this->settings['adminkey']));
        }
        ...
        elseif ($action == "new-service" && $action):
            if ($_POST):
                ...
                elseif (empty($want_username)):
                    ...
                    $dripfeed = $this->request->getPost('dripfeed')?$this->request->getPost('dripfeed'):1;
                    ...
                    if (isset($auto_price)) {
                        ...
                        if ($this->request->getPost('sync_rate_sabit') && $this->request->getPost('sync_rate_sabit') != "") {
                            $price = $this->request->getPost('sync_rate_sabit') + $price_api;
                            $sync_rate = $this->request->getPost('sync_rate_sabit');
                            $karoran = 0;
                        } else {
                            $price = round($topla, 2);
                            $karoran = 1;
                    ...
                    if ($insert):
                        $conn->commit();
                        $referrer = base_url($this->settings['adminkey']."/services");
                        $error = 1;
        ...
        elseif ($action == "edit-service" && $action):
            ...
            if (!countRow(["table" => "services", "where" => ["service_id" => $service_id]])): header("Location:" . base_url($this->settings['adminkey']."/services"));
                exit(); endif;
            ...
            if ($_POST):
                ...
                elseif (!is_numeric($price)):
                    ...
                    if ($this->request->getPost('ikiliislem')) {
                        ...
                        $update = $conn->prepare("UPDATE services SET birlestirme=:birles WHERE service_id=:id");
                        $update = $update->execute(array(
                            'id' => $serviceInfo['service_id'],
                            'birles' => 0,
                        ));
                    ...
                    if ($update):
                        ...
                        if ($min != $serviceInfo["service_min"]) {
                            if ($serviceInfo["service_type"] == 2) {
                                $servicereport->save(array(
                                    'service_id' => route(4),
                                    'alert' => $serviceInfo["service_name"],
                                    'extra' => json_encode([
                                        'action' => $min > $serviceInfo["service_min"] ? 'min_up' : 'min_down',
                                        'old' => $serviceInfo["service_min"],
                                        'new' => $min
                                    ])
                                ));
                        ...
                        if ($max != $serviceInfo["service_max"]) {
                            if ($serviceInfo["service_type"] == 2) {
                                $servicereport->save(array(
                                    'service_id' => route(4),
                                    'alert' => $serviceInfo["service_name"],
                                    'extra' => json_encode([
                                        'action' => $max > $serviceInfo["service_max"] ? 'max_up' : 'max_down',
                                        'old' => $serviceInfo["service_max"],
                                        'new' => $max
                                    ])
                                ));
                        ...
                        $errorText = "İşlem başarılı";
                        $icon = "success";
                        $referrer = $this->request->getGet('modalsave') ? -1 : base_url($this->settings['adminkey']."/services");
                    else:
                        $conn->rollBack();
                        $error = 1;
                        $errorText = "İşlem başarısız";
        ...
        elseif ($action == "getServiceInfo" && $action):
            $service_model = model('services');
            $getService = $service_model->where('service_id',route(4))->get()->getResultArray()[0];
            $data = [];
            $data['id'] = $getService['service_id'];
            $data['name'] = $getService['service_name'];
            $data['price'] = $getService['service_price'];
            $data['min'] = $getService['service_min'];
            $data['max'] = $getService['service_max'];
            $data['api_id'] = $getService['api_service'];
            if($getService['service_api'] != 0){
                $api_model = model("service_api");
                $api_cek = $api_model->where('id',$getService['service_api'])->get()->getResultArray()[0];
                $data['api_name'] = $api_cek['api_name'];
                $data['api_balance'] = json_decode($api_cek['api_json'],true)['balance'];
            }else{
                $data['api_name'] = "";
                $data['api_balance'] = "";
        ...
        elseif ($action == "new-category" && $action):
            if ($_POST):
                ...
                $rownames = $conn->prepare("SELECT * FROM categories WHERE category_name=:category_name");
                $rownames->execute(array("category_name" => $name));
                $rownames = $rownames->fetchAll(PDO::FETCH_ASSOC);
                if (empty($name)):
                    $error = 1;
                ...
                elseif ($rownames):
                    $error = 1;
                    $errorText = "Aynı isimle daha önceden kategori eklenmiş!";
                    $icon = "error";
                else:
                    $row = $conn->query("SELECT * FROM categories ORDER BY category_line DESC LIMIT 1 ")->fetch(PDO::FETCH_ASSOC);
                    $conn->beginTransaction();
                    $insert = $conn->prepare("INSERT INTO categories SET category_name=:name, category_line=:line, category_secret=:secret, icon=:ic,color=:co  ");
                    $insert = $insert->execute(array("name" => $name, "secret" => $secret, "line" => isset($row["category_line"]) ? $row["category_line"] + 1 : 1, "ic" => $icon, 'co' => $color));
                    if ($insert):
                        $conn->commit();
                        unset($_SESSION["data"]);
                        $error = 1;
        ...
        elseif ($action == "edit-category" && $action):
            $category_id = route(4);
            if (!countRow(["table" => "categories", "where" => ["category_id" => $category_id]])): header("Location:" . base_url($this->settings['adminkey']."/services"));
                exit(); endif;
            ...
            if ($_POST):
                ...
                if (empty($name)):
                    ...
                    $conn->beginTransaction();
                    $update = $conn->prepare("UPDATE categories SET category_name=:name, category_secret=:secret, icon=:ic,color=:co WHERE category_id=:id  ");
                    $update = $update->execute(array("name" => $name, "secret" => $secret, "id" => $category_id, "ic" => $icon, 'co' => $color));
                    if ($update):
                        $conn->commit();
                        $referrer = base_url($this->settings['adminkey']."/services");
                        $error = 1;
        ...
        elseif ($action == "edit-category-price" && $action):
            $category_id = route(4);
            if (!countRow(["table" => "categories", "where" => ["category_id" => $category_id]])): header("Location:" . base_url($this->settings['adminkey']."/services"));
                exit(); endif;
            $row = getRow(["table" => "categories", "where" => ["category_id" => $category_id]]);
            if ($_POST):
                $yuzde = $_POST["name"];
                $sabit = $_POST["sabit_kar"];
                $icon = isset($_POST["icon"]) ? $_POST["icon"] : "";
                $sabonelik = array(11,12,14, 15);

                if (empty($yuzde) && empty($sabit)):
                    $error = 1;
                    $errorText = "Yüzde ve Sabit Boş bırakılamaz";
                    $icon = "error";
                else:
                    $conn->beginTransaction();
                    $services_m = new \App\Models\services();
                    $bakiyeler = array();
                    $fiyat = $yuzde == 0 ? $sabit : $yuzde;
                    $sabitkar = $yuzde == 0 ? 0 : 1;
                    $servis_bul = $services_m->where('category_id', $category_id)->get()->getResultArray();
                    foreach ($servis_bul as $ss) {
                        if (in_array($ss['service_package'], $sabonelik)) {

                        } else {
                            $api_b = json_decode($ss['api_detail'], true);
                            if (isset($api_b['rate'])):
                                $yeni_bakiye = $yuzde ? $api_b['rate'] + $api_b['rate'] * $yuzde / 100 : $api_b['rate'] + $fiyat;
                                if ($settings["price_round"] == 1) {$yeni_bakiye = round($yeni_bakiye,$settings["price_rlimit"]);}
                                $set = array(
                                    'service_price' => $yeni_bakiye,
                                    'sync_rate' => $fiyat,
                                    'sync_price' => 1,
                                    'sync_kar_oran' => $sabitkar
                                );

                            else:
                                $yeni_bakiye = $yuzde ? $ss['service_price'] + $ss['service_price'] * $yuzde / 100 : $fiyat;
                                if ($settings["price_round"] == 1) {$yeni_bakiye = round($yeni_bakiye,$settings["price_rlimit"]);}
                                $set = array(
                                    'service_price' => $yeni_bakiye,
                                );
                            ...
                            if ($settings["price_round"] == 1) {$yeni_bakiye = round($yeni_bakiye,$settings["price_rlimit"]);}
                            $bakiyeler[$ss['service_id']] = $yeni_bakiye;
                            $services_m->protect(false)->where('service_id', $ss['service_id'])->set($set)->update();
                        }
                    }
                    $referrer = base_url($this->settings['adminkey']."/services");
                    $error = 1;
                ...
                echo json_encode(["t" => "error", "m" => $errorText, "s" => $icon, "r" => -1, 'service_toplu_bakiye' => json_encode($bakiyeler)]);
            endif;
        elseif ($action == "edit-category-aciklama" && $action):
            $category_id = route(4);
            if (!countRow(["table" => "categories", "where" => ["category_id" => $category_id]])): header("Location:" . base_url($this->settings['adminkey']."/services"));
                exit(); endif;
            $row = getRow(["table" => "categories", "where" => ["category_id" => $category_id]]);
            if ($_POST):
                ...
                if (empty($aciklama)):
                    $error = 1;
                    $errorText = "Açıklama Boş bırakılamaz";
                    $icon = "error";
                else:
                    $conn->beginTransaction();
                    $services_m = new \App\Models\services();
        ...
        elseif ($action == "edit-service-aciklama" && $action):
            $category_id = route(4);
            if (!countRow(["table" => "services", "where" => ["service_id" => $category_id]])): header("Location:" . base_url($this->settings['adminkey']."/services"));
                exit(); endif;
            $row = getRow(["table" => "services", "where" => ["service_id" => $category_id]]);
            if ($_POST):
                $aciklama = $_POST["name"];
                $icon = isset($_POST["icon"]) ? $_POST["icon"] : "";
                ...
                if (empty($aciklama)):
                    ...
                    $conn->beginTransaction();
                    $services_m = new \App\Models\services();
                    $servis_bul = $services_m->protect(false)->where('service_id', $category_id)->set("service_description", $aciklama)->update();
                ...
                echo json_encode(["t" => "error", "m" => $errorText, "s" => $icon, "r" => -1]);
            endif;
        elseif ($action == "edit-service-baslik" && $action):
            $category_id = route(4);
            if (!countRow(["table" => "services", "where" => ["service_id" => $category_id]])): header("Location:" . base_url($this->settings['adminkey']."/services"));
                exit(); endif;
            $row = getRow(["table" => "services", "where" => ["service_id" => $category_id]]);
            if ($_POST):
                ...
                if (empty($aciklama)):
                    $error = 1;
                    $errorText = "Başlık Boş bırakılamaz";
                    $icon = "error";
                else:
                    ...
                    foreach ($languages as $language) {
                        if ($language["default_language"]) {
                            $ss[$language["language_code"]] = $aciklama;
                        }

                    }
                    $servis_bul = $services_m->protect(false)->where('service_id', $category_id)->set(["service_name" => $aciklama, 'name_lang' => json_encode($ss)])->update();
                ...
                echo json_encode(["t" => "error", "m" => $errorText, "s" => $icon, "r" => -1, 'service_name' => $aciklama, 'service_id' => $category_id]);
            endif;
        ...
        elseif ($action == "service-active" && $action):
            $service_id = route(4);
            if (countRow(["table" => "services", "where" => ["service_id" => $service_id, "service_type" => 2]])): header("Location:" . base_url($this->settings['adminkey']."/services"));
                exit(); endif;
            $update = $conn->prepare("UPDATE services SET service_type=:type WHERE service_id=:id ");
            $update->execute(array("type" => 2, "id" => $service_id));
            $get_service = $conn->prepare("SELECT * FROM services WHERE service_id = ?");
            $get_service->execute([$service_id]);
            $get_service = $get_service->fetch(PDO::FETCH_ASSOC);
            $servicereport->save(array(
                'service_id' => $service_id,
                'alert' => $get_service['service_name'],
                'extra' => json_encode(["action" => "aktif"])
            ));
            if ($update):
            ...
            header("Location:" . base_url($this->settings['adminkey']."/services"));
        elseif ($action == "service-deactive" && $action):
            $service_id = route(4);
            if (countRow(["table" => "services", "where" => ["service_id" => $service_id, "service_type" => 1]])): header("Location:" . base_url($this->settings['adminkey']."/services"));
                exit(); endif;
            $update = $conn->prepare("UPDATE services SET service_type=:type WHERE service_id=:id ");
            ...
            $servicereport->save(array(
                'service_id' => $service_id,
                'alert' => $get_service['service_name'],
                'extra' => json_encode(["action" => "pasif"])
            ));
            if ($update):
                $_SESSION["client"]["data"]["success"] = 1;
                $_SESSION["client"]["data"]["successText"] = "İşlem başarılı";
        ...
        elseif ($action == "service-copy" && $action):
            ...
            return redirect()->to(base_url($this->settings['adminkey']."/services"));
        ...
        elseif ($action == "fiyat_sirala" && $action):
            $cat_id = route(4);
            $cat = $this->db->table("categories");
            $cat->where('category_id', $cat_id)->set('price_line', 1)->update();

            return redirect()->to(base_url($this->settings['adminkey']."/services"));
        elseif ($action == "fiyat_sirala_az" && $action):
            $cat_id = route(4);
            $cat = $this->db->table("categories");
            $cat->where('category_id', $cat_id)->set('fiyat_siralama', 1)->update();

            return redirect()->to(base_url($this->settings['adminkey']."/services"));
        elseif ($action == "fiyat_sirala_cok" && $action):
            $cat_id = route(4);
            $cat = $this->db->table("categories");
            $cat->where('category_id', $cat_id)->set('fiyat_siralama', 0)->update();

            return redirect()->to(base_url($this->settings['adminkey']."/services"));
        elseif ($action == "fiyat_sirala_iptal" && $action):
            $cat_id = route(4);
            $cat = $this->db->table("categories");
            $cat->where('category_id', $cat_id)->set('price_line', 0)->update();
        ...
        elseif ($action == "get_services_add" && $action):
            ...
            if (count($services)):
                foreach ($services as $service => $price):
                    foreach ($apiServices as $apiService):
                        if ($service == $apiService->service && $service != 0):
                            ...
                            if (isset($apiService->description)) {
                                $desc = htmlspecialchars_decode($apiService->description);
                            }
                            $detail["currency"] = $balance->currency;
                            ...
                            //if ($package == 11 || $package == 12 || $package == 14 || $package == 15):
                            if ($package == 11):
                                $insert = $conn->prepare("INSERT INTO services SET 
                      service_api=:api,
                      api_service=:api_service,
                      ...
                      service_description=:desc
                       ");
                                $insert = $insert->execute(array(
                                    ...
                                    "lang" => $name3,
                                    "price" => $price,
                                    "min" => isset($apiService->min) ? $apiService->min : 0,
                                    "max" => isset($apiService->max) ? $apiService->max : 0,
                    ...
                    endforeach;
                endforeach;
                echo json_encode(["t" => "error", "m" => "İşlem başarılı", "s" => "success", "r" => base_url($this->settings['adminkey']."/services"), "time" => 0]);
            else:
    ...
    function detail()
    {
        ...
        if ($this->request->uri->getSegment(3)) {
            ...
            if ($this->request->getGet('modal')) {
                return view('admin/yeni_admin/service-detail-modal', $ayar);
    ...
    function services_ajax()
    {
        ...
        if (empty($action)):
            ...
            $services = $conn->prepare("SELECT * FROM categories ORDER BY category_line");
            ...
            foreach ($services as $ss) {
                $where = $bul->where('services.category_id', $ss['category_id'])->join("categories", "categories.category_id = services.category_id")->limit(1)->get()->getResultArray();
                $serviceList[$ss['category_name']] = $where;
    ...
    function gizle_ajax()
    {
        ...
        if (empty($action)):
            ...
            $cat = $this->db->table("categories");
            $siralama = $cat->where('category_id', $category_id)->get()->getResultArray()[0];
            if (!$siralama['price_line']):
                $services = $conn->prepare("SELECT * FROM services RIGHT JOIN categories ON categories.category_id = services.category_id LEFT JOIN service_api ON service_api.id = services.service_api WHERE services.category_id = ? ORDER BY categories.category_line,services.service_line ");
            else:
                $services = $conn->prepare("SELECT * FROM services RIGHT JOIN categories ON categories.category_id = services.category_id LEFT JOIN service_api ON service_api.id = services.service_api WHERE services.category_id = ? ORDER BY categories.category_line,services.service_price DESC");
            endif;
            $services->execute(array($category_id));
            $services = $services->fetchAll(PDO::FETCH_ASSOC);
            $serviceList = array_group_by($services, 'category_name');
            $ayar = array(
                'title' => 'Services',
            ...
            //return view('admin/services', $ayar);
            return view('admin/yeni_admin/services-goster-ajax', $ayar);
