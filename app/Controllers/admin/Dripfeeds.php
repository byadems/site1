...
class Dripfeeds extends Ana_Controller
{
    ...
    function index()
    {
        ...
        $settings = $this->settings;
            $access = $this->getuser["access"]["dripfeed"];
    if (!$access) {
  ...
  if( route(3) ==  "dripfeed_canceled" && route(3)):
      $update = $conn->prepare("UPDATE orders SET dripfeed_status=:status WHERE order_id=:id ");
      $update->execute(array("status"=>"canceled","id"=>route(4)));
      header("Location:".base_url($settings['adminkey']."/dripfeeds"));
  elseif( route(3) ==  "dripfeed_completed"  && route(3)):
      $update = $conn->prepare("UPDATE orders SET dripfeed_status=:status WHERE order_id=:id ");
      $update->execute(array("status"=>"completed","id"=>route(4)));
      header("Location:".base_url($settings['adminkey']."/dripfeeds"));
  elseif( route(3) ==  "dripfeed_canceledbalance"  && route(3)):
    $id     = route(4);
