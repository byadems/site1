...
class Appearance extends Ana_Controller
{
    function index()
    {
        ...
        $accesspages = $this->getuser["access"]["pages"];
        if ($accesspages) {
            $menuList += ['Sayfalar' => 'pages'];
        }
        $accessnews = $this->getuser["access"]["news"];
        if ($accessnews) {
            $menuList += ['Duyurular' => 'news'];
        }
        $accessblog = $this->getuser["access"]["blog"];
        if ($accessblog) {
            $menuList += ['Blog' => 'blog'];
        }
        $accessmenu = $this->getuser["access"]["menu"];
        if ($accessmenu) {
            $menuList += ['Menü' => 'menu'];
        }
        $accessthemes = $this->getuser["access"]["themes"];
        if ($accessthemes) {
            $menuList += ['Temalar' => 'themes'];
        }
        $accesslanguage = $this->getuser["access"]["language"];
        if ($accesslanguage) {
            $menuList += ['Diller' => 'language'];
        }
        $accessfiles = $this->getuser["access"]["files"];
        if ($accessfiles) {
            $menuList += ['Dosyalar' => 'files'];
        }
        ...
        if (!array_search(route(3), $menuList)) :
            header("Location:" . base_url($settings['adminkey'] . "/appearance"));
        ...
        elseif (route(3) == "pages" or $route[2] == "pages"):
            ...
            if ($access):
                if (route(4) == "edit" && route(4)):
                    ...
                    $ayar['page'] = $page;
                    if (!$page): header("Location:" . base_url($settings['adminkey']."/appearance/pages")); endif;
                elseif (!route(4)):
                    ...
                    header("Location:" . base_url($settings['adminkey']."/appearance/pages"));
                endif;
            endif;
            if (1 == 0): header("Location:" . base_url($settings['adminkey']."/appearance/pages")); endif;
        ...
        elseif (route(3) == "menu" && route(3)):
            ...
            if (route(4) == "add"):
                if ($this->request->getPost('menu_name') && $this->request->getPost('menu_icon') && $this->request->getPost('menu_link')):
                    ...
                    header("Location:" . base_url($settings['adminkey']."/appearance/menu"));
                endif;
            endif;
            ...
            if ($access):
                ...
                if ($id):
                    ...
                    elseif (route(4) == "delete" && route(4)):
                        ...
                        if (!countRow(["table" => "menu", "where" => ["id" => $id]])):
                            ...
                            if ($delete):
                                ...
                                $referrer = base_url($settings['adminkey']."/settings/menu");
                            else:
                                $error = 1;
                                $icon = "error";
                    ...
                    elseif (route(4) == "edit" && route(4)):
                        ...
                        $post->execute(array("id" => route(5)));
                        $post = $post->fetch(PDO::FETCH_ASSOC);
                        if (!$post): header("Location:" . base_url($settings['adminkey']."/appearance/menu"));
                        endif;
        ...
        elseif (route(3) == "blog"):
            ...
            if ($access):
                ...
                if (route(4) == "edit" && route(4)):
                    ...
                    $post->execute(array("id" => route(5)));
                    $post = $post->fetch(PDO::FETCH_ASSOC);
                    if (!$post): header("Location:" . base_url($settings['adminkey']."/appearance/blog"));
                    endif;
                    ...
                    if ($_POST):
                        ...
                        elseif (empty($name)):
                            ...
                            if ($insert):
                                ...
                                $referrer = base_url($settings['adminkey']."/settings/blog");
                            else:
                                $error = 1;
                                $errorText = "İşlem başarısız";
                            endif;
                ...
                elseif (route(4) == "delete" && route(4)):
                    ...
                    header("Location:" . base_url($settings['adminkey']."/appearance/blog"));
                    exit();
                else:
                    header("Location:" . base_url($settings['adminkey']."/appearance/blog"));
                endif;
            endif;
            if (1 == 0): header("Location:" . base_url($settings['adminkey']."/appearance/blog")); endif;
        ...
        elseif (route(3) == "language"):
            ...
            if ($access):
                ...
                if (route(4) && route(4) != "new" && !countRow(["table" => "languages", "where" => ["language_code" => route(4)]])):
                    header("Location:" . base_url($settings['adminkey']."/appearance/language"));
                ...
                if ($_POST && route(4) != "new" && countRow(["table" => "languages", "where" => ["language_code" => route(4)]])):
                    ...
                    //sleep(2);
                    include APPPATH . 'ThirdParty/dil/' . route(4) . '.php';
                    return json_encode(["t" => "error", "m" => "Başarıyla Dil Düzenlendi", "s" => "success", "r" => base_url($settings['adminkey']."/appearance/language/" . route(4))]);
            ...
            if ($access):
                ...
                elseif (route(4) && route(4) && countRow(["table" => "themes", "where" => ["theme_dirname" => route(4)]])):
                    ...
                    if ($_POST):
                        ...
                        fclose($updated_file);
                        header("Location:" . base_url($settings['adminkey']."/appearance/themes/" . $theme["theme_dirname"] . "?file=" . $lyt));
                    endif;
                elseif ("" && !countRow(["table" => "themes", "where" => ["theme_dirname" => ""]])):
                    header("Location:" . base_url($settings['adminkey']."/appearance/themes"));
                else:
                    $themes = $conn->prepare("SELECT * FROM themes ORDER BY id DESC");
                    $themes->execute(array());
                    $themes = $themes->fetchAll(PDO::FETCH_ASSOC);
            ...
            if ($access):
                ...
                if (route(4) == "new" && route(4) && $_POST):
                    ...
                    elseif (empty($content)):
                        ...
                        if ($insert):
                            $conn->commit();
                            $referrer = base_url($settings['adminkey']."/appearance/news");
                            $error = 1;
                            $errorText = "İşlem başarılı";
                            $icon = "success";
                        else:
                ...
                elseif (!route(4)):
                    ...
                    header("Location:" . base_url($settings['adminkey']."/appearance/news"));
                endif;
            endif;
            if (1 == 0): header("Location:" . base_url($settings['adminkey']."/appearance/news")); endif;
        ...
        elseif (route(3) == "files"):
            ...
            if ($access):
                ...
                if (route(4) == "delete" && route(4)):
                    ...
                    header("Location:" . base_url($settings['adminkey']."/appearance/files"));
                    exit();
                endif;
                //1
            ...
            if (1 == 0): header("Location:" . base_url($settings['adminkey']."/appearance/files"));
            endif;
