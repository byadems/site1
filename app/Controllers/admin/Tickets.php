...
class Tickets extends Ana_Controller
{
    function index()
    {
        ...
        $count = 1;
        $access = $this->getuser["access"]["tickets"];
        if (!$access) {
        ...
        elseif (route(3) == "read" && route(3)):
            if (!countRow(["table" => "tickets", "where" => ["ticket_id" => route(4)]])): header("Location:" . base_url($this->settings['adminkey']."/tickets"));
                exit(); endif;
            if ($_POST):
                ...
                if (strlen($message) < 3):
                    ...
                    if ($insert && $update):
                        $conn->commit();
                        header("Location:" . base_url($this->settings['adminkey']."/tickets/read/" . route(4)));
        ...
        elseif (route(3) == "unread" && route(3)):
            ...
            if ($update):
                header("Location:" . base_url($this->settings['adminkey']."/tickets"));
                $_SESSION["client"]["data"]["success"] = 1;
                $_SESSION["client"]["data"]["successText"] = "İşlem başarılı";
            else:
                header("Location:" . base_url($this->settings['adminkey']."/tickets"));
        ...
        elseif ($action == "new" && $action):
            if ($_POST):
                ...
                elseif (empty($message)):
                    ...
                    if ($insert && $insert2):
                        $conn->commit();
                        $referrer = base_url($this->settings['adminkey']."/tickets");
                        $error = 1;
    ...
    function read()
    {
        ...
        if (!countRow(["table" => "tickets", "where" => ["ticket_id" => $this->request->uri->getSegment(4)]])): header("Location:" . base_url($this->settings['adminkey']."/tickets"));
            exit(); endif;
        ...
        if ($_POST):
            ...
            if (strlen($message) < 3):
                ...
                if ($insert && $update):
                    ...
                    /********************************************************************************/
                    header("Location:" . base_url($this->settings['adminkey']."/tickets/read/" . $this->request->uri->getSegment(4)));
                    $ayar["success"] = 1;
