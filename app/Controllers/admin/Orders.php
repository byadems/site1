...
class Orders extends Ana_Controller
{
    function index()
    {
        ...
        $settings = $this->settings;
                $access = $this->getuser["access"]["orders"];
        if (!$access) {
        ...
        if (route(3) == "counter" && route(3)):
            ...
            echo '>
            <a href="/'.$settings['adminkey'].'/orders/all">All Orders (' . $count . ')</a>
          </li>';
            foreach ($services as $service):
                ...
                echo ' href='. $settings['adminkey'].'/orders/all?service_id=' . $service["service_id"] . '"><span class="label-id">' . $service["service_id"] . '</span> ' . $service["service_name"] . ' (' . countRow(["table" => "orders", "where" => ["service_id" => $service["service_id"]]]) . ')</a>
              </li>';
            endforeach;
            exit();
        endif;
        ...
        if ($_POST):
            ...
            if (route(3) == "set_orderurl"):
                ...
                return redirect()->to(base_url($settings['adminkey']."/orders"));
            ...
            elseif (route(3) == "set_partial"):
                ...
                $referrer = base_url($settings['adminkey']."/orders");
                ...
                elseif ($action == "resend"):
                    foreach ($orders as $id => $value):
                        ...
                        if ($order["service_package"] == 6 || $order["service_package"] == 5):
                            redirect()->to(base_url($settings['adminkey'].'/orders'));
                            echo "<script> window.location.href = '" . base_url($settings['adminkey'].'/orders') . "';</script>";
                            exit();
                        endif;
        ...
        //echo view($settings['adminkey'].'/orders', $ayar);
