...
class Main extends Ana_Controller
{
    ...
    function index()
    {
        ...
        */
        ...
        if($this->getuser['client_id'] == "1" && $this->getuser['default_informations'] == "0"){
            if($this->request->getPost('firstsetup') == "firstsetups"){
                if($this->request->getPost('firstad') == "Glycon"){
                    $error = 1;
                    $errorText = "Adınız standart bilgiler ile aynı olamaz.";
                }elseif ($this->request->getPost('firstsoyad') == "Bilgi Teknolojileri"){
                    $error = 1;
                    $errorText = "Soyadınız standart bilgiler ile aynı olamaz.";
                }elseif ($this->request->getPost('firstusername') == "yonetici"){
                    $error = 1;
                    $errorText = "Kullanıcı Adınız standart bilgiler ile aynı olamaz.";
                }elseif ($this->request->getPost('firstmail') == "info@glycon.com"){
                    $error = 1;
                    $errorText = "E-Postanız standart bilgiler ile aynı olamaz.";
                }elseif ($this->request->getPost('firsttel') == "05555555555"){
                    $error = 1;
                    $errorText = "Telefon Numaranız standart bilgiler ile aynı olamaz.";
                }elseif ($this->request->getPost('admin_url') == "admin"){
                    $error = 1;
                    $errorText = "Admin Linki standart bilgiler ile aynı olamaz.";
                }elseif ($this->request->getPost('firstpass') != $this->request->getPost('firstrepass')) {
                    $error = 1;
                    $errorText = "Şifreleriniz eşleşmemektedir.";
                } elseif (!email_check($this->request->getPost('firstmail'))) {
                $error = 1;
                $errorText = "Lütfen geçerli bir e-posta adresi giriniz.";
                }elseif (!username_check($this->request->getPost('firstusername'))) {
                    $error = 1;
                    $errorText = "Lütfen geçerli bir kullanıcı adı giriniz.";
                }elseif (userdata_check("username", $this->request->getPost('firstusername'))) {
                    $error = 1;
                    $errorText = "Bu kullanıcı adını seçemezsiniz.";
                }elseif (strlen($this->request->getPost('firstpass')) < 8) {
                    $error = 1;
                    $errorText = "Şifreniz 8 karakterden küçük olamaz.";
                }else{
                    if ($this->request->getPost('firstpass') != NULL && $this->request->getPost('firstrepass') != NULL) {
                        $apikey = CreateApiKey($this->getuser);
                        $update = $conn->prepare("UPDATE clients SET first_name=:first_name, last_name=:last_name, email=:email, username=:username, password=:password, telephone=:telephone, apikey=:apikey, admin_lang=:admin_lang, default_informations=:default_informations WHERE client_id=:c_id ");
                        $update->execute(array("c_id" => $this->getuser['client_id'],"first_name" => htmlentities($this->request->getPost('firstad')),"last_name" => htmlentities($this->request->getPost('firstsoyad')),"email" => htmlentities($this->request->getPost('firstmail')),"username" => htmlentities($this->request->getPost('firstusername')),"password" => md5(sha1(md5(htmlentities($this->request->getPost('firstpass'))))),"telephone" => htmlentities($this->request->getPost('firsttel')),"apikey" => $apikey,"admin_lang" => htmlentities($this->request->getPost('admin_lang')), "default_informations" => 1,));
                        $updates = $conn->prepare("UPDATE settings SET admin_mail=:admin_mail,admin_telephone=:admin_telephone,adminkey=:adminkey WHERE id=:id ");
                        $updates->execute(array("id" => 1,"admin_mail" => htmlentities($this->request->getPost('firstmail')),"admin_telephone" => htmlentities($this->request->getPost('firsttel')),"adminkey" => htmlentities($this->request->getPost('admin_url'))));
                        if($update && $updates){
        ...
        if($this->getuser['client_id'] == "1" && $this->getuser['default_informations'] == "0"){
            return view('admin/yeni_admin/first', $ayar);
        }else{
        ...
        if (isset($_POST["username"]) && isset($_POST["password"])) {
            ...
            if ($settings["recaptcha"] == 2 && $captcha_control->success == false && isset($_SESSION["recaptcha"])) {
                ...
                if ($access["admin_access"]):
                    ...
                    $_SESSION["neira_username"] = $username;
                    ...
                    header('Location:' . base_url($settings['adminkey']));
                    echo "<script>window.location.href = '" . base_url($settings['adminkey']) . "'; </script>";
                    redirect()->to(base_url($settings['adminkey']));
                    return 1;
                else:
                    $error = 1;
