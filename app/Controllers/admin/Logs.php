...
class Logs extends Ana_Controller
{
    function index()
    {
        ...
        if (route(3) && is_numeric(route(3))):
            ...
            $page = 1;
                ...
                $access = $this->getuser["access"]["logs"];
        if (!$access) {
        ...
        if (route(3) == "delete" && route(3)):
            ...
            header("Location:" . base_url($settings['adminkey']."/logs"));
    ...
    function provider()
    {
        ...
        global $_SESSION;
        $search_add = "";
                $access = $this->getuser["access"]["provider_logs"];
        if (!$access) {
            header("Location:" . base_url($this->settings['adminkey']));
        }
        if (route(3) && is_numeric(route(3))):
        ...
        elseif (route(3) == "multi-action" && route(3)):
            ...
            foreach ($logs as $id => $value):
                ...
                $delete->execute(array("id" => $id));
            endforeach;
            header("Location:" . base_url($settings['adminkey']."/provider_logs"));
        endif;
    ...
    function guard()
    {
        global $conn;
        global $_SESSION;
                $access = $this->getuser["access"]["guard_logs"];
        if (!$access) {
            header("Location:" . base_url($this->settings['adminkey']));
        }
        $search_add = "";
        ...
        elseif (route(3) == "multi-action" && route(3)):
            ...
            foreach ($logs as $id => $value):
                ...
                $delete->execute(array("id" => $id));
            endforeach;
            header("Location:" . base_url($settings['adminkey']."/guard_logs"));
        endif;
    ...
    function tasks()
    {
        global $conn;
        global $_SESSION;
                $access = $this->getuser["access"]["tasks"];
        if (!$access) {
            header("Location:" . base_url($this->settings['adminkey']));
        }
        $settings = $this->settings;
        ...
        if (route(3) && route(3) == "no") {
            ...
            header("Location:" . base_url($settings['adminkey']."/tasks"));
            ...
            if ($settings["auto_refill"] != 2):
                ...
                if (isset($order['api_key'])):
                    ...
                    if (@$send_refill->refill):
                        ...
                        $error = 1;
                        $errorText = "Talebiniz gönderilemedi. Hata Kodu:<code>" . $send_refill . "</code>";
                    endif;
                endif;
            endif;
