...
function encrypt_data($data, $secret_key) {
    ...
    $encrypted = openssl_encrypt($json, 'aes-256-cbc', $secret_key, 0, $iv);
    return base64_encode($iv . $encrypted);
}
    
function decrypt_data($encrypted_data, $secret_key) {
    $encrypted_data = base64_decode($encrypted_data);
    $iv = substr($encrypted_data, 0, openssl_cipher_iv_length('aes-256-cbc'));
    $encrypted = substr($encrypted_data, openssl_cipher_iv_length('aes-256-cbc'));
...
function service_price($service,$user_id = null)
{
    global $conn;
    global $user;
    $session = \Config\Services::session();
    if(!is_null($user_id) || ($session->get('neira_userlogin') && $session->get('neira_userlogin') == 1)){
    
    
    $userid = $session->get('neira_userid');
    
    if(!is_null($user_id)) $userid = $user_id;
    $row = $conn->prepare("SELECT * FROM clients_price WHERE service_id=:s_id && client_id=:c_id ");
    $row->execute(["s_id" => $service, "c_id" => $userid]);
    $db = \Config\Database::connect();
    $row = $db->table("clients_price")->where(array(
